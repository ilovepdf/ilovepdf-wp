...
trait Actions {
    ...
    protected function render_compress_action( $post_id ) {
        return sprintf(
            '<a id="ipdf-action-compress-" data-post-id="%4$s" data-action="%5$s" data-nonce="%1$s" href="#" class="ipdf-btn ipdf-tooltip ipdf-btn--media-action ipdf-btn--media-action-compress %6$s">
                %2$s
    ...
    protected function status_not_compressed( $post_id ) {
        return sprintf(
            ...
            esc_html_x( 'Not compressed', '', 'ilove-pdf' ),
        );
    }
    ...
    protected function status_compressing() {
        return sprintf(
            ...
            $this->get_loading(),
            esc_html_x( 'Compressing...', '', 'ilove-pdf' ),
        );
    }
