...
(function ($) {
	...
	$('#check-all-compress').on('click', function () {
		if (this.checked) {
			$('.nslb-input').each(function () {
				this.checked = true;
			});
		} else {
			$('.nslb-input').each(function () {
	...
	$('#check-all-watermark').on('click', function () {
		if (this.checked) {
			$('.nslb-input').each(function () {
				this.checked = true;
			});
				...
				if (response != '' && response.status == 1 && !response.api_error) {
					...
					if (!jQuery.isEmptyObject(list_pdf) && xhr) {
						...
						window.setTimeout(function () {
							...
							$('#stats_total_percentage')
								.parent('.c100')
								.addClass('p' + response.percentage);
					...
					if (response.api_error == 'error_auth') {
						tb_show(
							'HAI',
							'#TB_inline?height=240&amp;width=405&amp;inlineId=pricing_ilovepdf&amp;modal=true',
							null
						);
						elem.show();
					} else {
	...
	function watermarkMultiPDF(list_pdf) {
		...
		$('.all-applying-watermark .progress-percent').width(
			(tmp_count * 100) / array_lenght + '%'
		);
		$.post(
	...
	$('.btn-compress').on('click', function (e) {
		var elem = $(this);
		...
		$.post(
			...
			function (response) {
				...
				if (response != '' && response.status == 1 && !response.api_error) {
					...
					$('#stats_total_files_compressed').html(
						parseInt($('#stats_total_files_compressed').html()) + 1
					);
					$('#stats_initial_size').html(response.initial_size);
					$('#stats_current_size').html(response.current_size);
					$('#stats_total_percentage').html(response.percentage + '%');
					...
					window.setTimeout(function () {
						...
						if (response.library == 1) {
							elem.nextAll('.stats-compress').html(
								'<i class="fa fa-check" aria-hidden="true"></i> Compressed<br />Savings ' +
									response.percent +
									'%</span>'
							);
						} else if (response.editpdf == 1) {
							elem.prevAll('#current-size')
								.find('strong')
								.html(response.compress_size);
						} else {
	...
	$('.btn-watermark').on('click', function (e) {
		var elem = $(this);
		...
		$.post(
			...
			function (response) {
				...
				if (response != '' && response.status == 1 && !response.api_error) {
					elem.nextAll('.success').show();
					$('#stats_total_files_watermarked').html(
						parseInt($('#stats_total_files_watermarked').html()) + 1
					);
					window.setTimeout(function () {
						elem.nextAll('.success').hide();
						if (response.library == 1) {
							parent_elem.html(
								'<i class="fa fa-check" aria-hidden="true"></i> Stamped'
							);
						} else if (response.editpdf == 1) {
							parent_elem.append(
								'<i class="fa fa-check" aria-hidden="true"></i> Stamped'
	...
	$('.btn-restore').on('click', function (e) {
		...
		btnConfirmDialog.addEventListener('click', (e) => {
			...
			$.post($(this).prop('href') + '&ajax=1', function (response) {
